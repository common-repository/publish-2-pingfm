...
/*
 * Last change : $Date: 2010-05-04 08:42:05 -0500 (Tue, 04 May 2010) $ by $Author: foux $
 * Revision : $Revision: 237423 $
 */
...
if (! class_exists("Publish2PingFMAdminPanel")) {
	class Publish2PingFMAdminPanel {
		...
		public function registerAdminPanel() {
			$settingsName = __('Publish 2 Ping.fm Settings','publish2pingfm');
			$pluginName = __('Publish 2 Ping.fm','publish2pingfm');
			$page = add_submenu_page('options-general.php',
									 $settingsName,
									 $pluginName, 9, basename(__FILE__),
...
<form method="post" name="publish2pingform" action="<?php echo $_SERVER["REQUEST_URI"]; ?>">
	<div>
		<div class="wrap">
			<h2><?php _e('Wordpress 2 Ping.fm Settings','publish2pingfm'); ?></h2>
			<p><?php _e('In order to get Wordpress 2 Ping.fm work, you need a <a href="http://www.ping.fm">Ping.fm account</a>, a <a href="http://bit.ly">bit.ly</a> account, and to fill in the settings.','publish2pingfm'); ?></p>
			
			<table class="form-table">
				<tr valign="top">
					<th scope="row">
						<?php _e('Ping.fm API Key','publish2pingfm'); ?>
					</th>
					<td>
				...
				<tr valign="top">
					<th scope="row">
						<?php _e('Bit.ly user name','publish2pingfm'); ?>
					</th>
					<td>
				...
				<tr valign="top">
					<th scope="row">
						<?php _e('Bit.ly API Key','publish2pingfm'); ?>
					</th>
					<td>
				...
				<tr valign="top">
					<th scope="row">
						<?php _e('Status templates','publish2pingfm'); ?>
					</th>
					<td><?php 
				...
				<tr valign="top">
					<th scope="row">
						<?php _e('Categories','publish2pingfm'); ?>
					</th>
					<td>
						<table><?php 
							// Getting each categories
...
?></select></div>
				...
				if ($wpPingPrefs->isEnabled($catId)) {
					...
					echo "hidden";
				}?>"><?php _e('Publish post updates?','publish2pingfm'); ?><input type="checkbox" value="<?php 
